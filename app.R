...
## == Server Setting ==
fileName = "PERM_Disclosure_Data_FY2016_2020.xlsx"

# PDF Report object
pdfReport <- ggplot() + theme_void() + geom_text(aes(0,0,label=''))

### === Source data for the application === 
# = Load data from PERM_Disclosure_Data and construct DF =
myFile <-
  read_excel(fileName,
             sheet = 1,
             col_names = T)
    ...
    tabPanel(h4("Skill Matching"),
             dashboardPage(
               ...
               dashboardBody(
                 br(),
                 fluidRow(
                   uiOutput("cloud_resume", width = 12)
                 ),
                 fluidRow(
                   uiOutput("cloud_recommendations", width = 12)
                 ),
                 fluidRow(
